...
{	
	...
	return func_container->is_before(*auxdata1, *auxdata2);
}
...
static void Swap(void **front, void **back)
{
	void *temp = NULL;
	temp = *front;
	*front = *back;
...
static void *Remove(pqueue_t *pqueue, size_t index)
{
	...
	/*if(pqueue->nmemb > 1 || index == pqueue->nmemb - 1)
	{*/
		/*swap front and back*/
...
void *PQueuePeek(pqueue_t *pqueue)
{
	...
	assert(pqueue != NULL);
	assert(pqueue->vector != NULL);
	data_to_return = DynVecGetItemAddress(pqueue->vector, 0);
	return *data_to_return;
}
...
{	
	...
	{
		current_pointer = DynVecGetItemAddress(pqueue->vector, i);
		if(is_match(*current_pointer, params, data_to_match))
		{
			data_to_return = Remove(pqueue, i);
			--i;
		}
	}
	return data_to_return;
