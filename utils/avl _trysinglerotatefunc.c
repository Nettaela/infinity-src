...
{
	return GetNodeHeight(LEFT_SON(node)) - GetNodeHeight(RIGHT_SON(node));
}
...
static node_t *Balance(node_t *node) /*always diff is left - right*/
{
	...
	heightdiff = HeightDiff(node);
		
	if(heightdiff == 2 || heightdiff == -2) 
	{
		side = (2 == heightdiff) ? RIGHT : LEFT;
		/*if(heightdiff == 2)
		{
			side = RIGHT;
		}
		...
		heightdiff_grandchildren = HeightDiff(node->sons[!side]);
		
		if(heightdiff_grandchildren <= -1 || heightdiff_grandchildren >= 1)
		{
			node->sons[!side] = Rotate(node->sons[!side], !side);	
		}
